// Default variables for Instagram Profile template
...
// Storage functions
const STORAGE_KEYS = {
    VARIABLES: 'insta_profile_editor_variables',
    IMAGES: 'insta_profile_editor_images'
};
...
function initializeVariables() {
    ...
    panelInputs.forEach(input => {
        ...
        // show default when no user value exists
...
function renderTemplate() {
    ...
    textElements.forEach(element => {
        ...
        const variableValue = appState.variables[variableName];

        if (currentText.includes('{') || currentText.includes('}')) {
            element.textContent = substituteVariables(currentText);
            return;
        }

        if (variableValue !== undefined && variableValue !== null && String(variableValue).trim() !== '') {
            element.textContent = variableValue;
        } else {
            element.textContent = DEFAULT_VARIABLES[variableName] || currentText;
        }
    });
...
async function loadTemplateImages() {
    const imageSlots = ['profileImage', 'story1', 'story2', 'story3', 'grid1', 'grid2', 'grid3', 'grid4', 'grid5', 'grid6', 'grid7', 'grid8', 'grid9', 'bottomProfileImage'];
...
function setupImageHandlers() {
    ...
    const placeholders = document.querySelectorAll('.image-placeholder, .story-placeholder');
...
async function exportProject() {
    try {
        ...
        const link = document.createElement('a');
        link.download = `insta-profile-project-${Date.now()}.json`;
        link.href = url;
        link.click();
