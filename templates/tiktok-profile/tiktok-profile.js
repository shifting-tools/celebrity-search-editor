// Default variables for TikTok Profile template
...
// Storage functions
const STORAGE_KEYS = {
    VARIABLES: 'tiktok_profile_editor_variables',
    IMAGES: 'tiktok_profile_editor_images'
};
...
async function loadTemplateImages() {
    const imageSlots = ['profileImage', 'video1', 'video2', 'video3', 'video4', 'video5', 'video6'];
...
function setupRandomizeViews() {
    const randomizeBtn = document.getElementById('randomizeViewsBtn');
    if (!randomizeBtn) return;

    randomizeBtn.onclick = () => {
        const min = Math.floor(Math.random() * 900) + 100;
        const max = min + Math.floor(Math.random() * 900) + 100;

        appState.variables.viewsMin = `${min}K`;
        appState.variables.viewsMax = `${max}K`;
        appState.variables.video1Views = `${Math.floor(min + (max - min) * 0.45)}K`;
        appState.variables.video2Views = `${Math.floor(min + (max - min) * 0.72)}K`;
        appState.variables.video3Views = `${Math.floor(min + (max - min) * 0.91)}K`;
        appState.variables.video4Views = `${Math.floor(min + (max - min) * 0.32)}K`;
        appState.variables.video5Views = `${Math.floor(min + (max - min) * 0.22)}K`;
        appState.variables.video6Views = `${Math.floor(min + (max - min) * 0.15)}K`;
...
async function exportToPNG() {
    ...
    try {
        ...
        link.download = `tiktok-profile-${Date.now()}.png`;
        link.href = dataUrl;
        link.click();
    } catch (error) {
...
async function exportProject() {
    try {
        ...
        const link = document.createElement('a');
        link.download = `tiktok-profile-project-${Date.now()}.json`;
        link.href = url;
        link.click();
